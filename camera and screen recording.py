### import pyxhook (thats probably a module for keyboard events, I've never tried it)

import  cv2, pyautogui, datetime
import numpy as np


### this block is an idea to create keyboard shortcuts to control the camera and/or screenrecording: 
...
cap = cv2.VideoCapture("http://10.0.0.170:8081/")   ### cv2.VideoCapture(0)

print("starting")

while True:
    _, img = cap.read()
    cv2.imshow("img", img)   ## cv2.imshow("img", cv2.flip(img, 1))
    cv2.waitKey(1)
cap.release()
...
#out = cv2.VideoWriter('r"desktop\capture_size_function_camera.avi', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
#while(cap.isOpened()):
#       ret, frame = cap.read()
#       out.write(frame)
#       if cv2.waitKey(1) == ord("q"):
...
def audiocapture():
    ## this fuction is not working lol
  import pyaudio
  import wave
